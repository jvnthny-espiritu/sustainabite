    ...
    const fetchDataFromFirebase = async (searchTerm: string) => {
      try {
        ...
        const filteredPosts = postsData.filter((post) => {
          ...
          // Check for a match in post fields
          const postFieldsMatch =
            searchTermMatch(post.description) ||
            searchTermMatch(post.title) ||
            searchTermMatch(post.location) ||
            searchTermMatch(post.selectedCategory);
    
          // Check for a match in user fields (username and name)
          const user = users[post.userId];
          const userFieldsMatch =
            user &&
            (searchTermMatch(user.username) || searchTermMatch(user.name));
    
          return postFieldsMatch || userFieldsMatch;
        });
